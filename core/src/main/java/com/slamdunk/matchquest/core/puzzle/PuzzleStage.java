...
import static com.slamdunk.matchquest.core.MatchQuest.screenHeight;
import static com.slamdunk.matchquest.core.MatchQuest.screenWidth;
...
import com.slamdunk.matchquest.core.puzzle.PuzzleSwitchInputProcessor.SwitchListener;
...
public class PuzzleStage extends Stage implements SwitchListener {
	...
	public void render(float delta) {
		...
		drawCharts(50, 500, 25, 6);
		drawCharts(100, 500, 25, 5);
		drawCharts(150, 500, 25, 4);
		drawCharts(200, 500, 25, 3);
		drawCharts(250, 500, 25, 2);
		drawCharts(300, 500, 25, 1);
	}

	private void drawCharts(float centerX, float centerY, float radius, int nbParts) {
		...
		final float partAngle = (float)(360 / nbParts);
		shapeRenderer.setProjectionMatrix(getCamera().combined);

		shapeRenderer.begin(ShapeType.Line);
		for (int curPart = 0; curPart < nbParts; curPart++) {
			shapeRenderer.setColor(colors[curPart]);
			shapeRenderer.arc(centerX, centerY, radius, partAngle * curPart, partAngle, 18);
		}
		shapeRenderer.end();
		
		// Remplissage des morceaux
		shapeRenderer.begin(ShapeType.Filled);
		for (int curPart = 0; curPart < nbParts; curPart++) {
			shapeRenderer.setColor(colors[curPart]);
			shapeRenderer.arc(centerX, centerY, radius, partAngle * curPart, partAngle * fillPct[curPart], 18);
		}
		shapeRenderer.end();
	}

	private Vector2[] computeVertices(float centerX, float centerY, float radius, int nbParts) {
		final double partAngle = 2 * Math.PI / nbParts;
		Vector2[] vertices = new Vector2[nbParts];
		double curAngle;
		for (int curPart = 0; curPart < nbParts; curPart++) {
			curAngle = partAngle * curPart;
			Vector2 vertex = new Vector2();
			vertex.x = (float)(centerX + Math.cos(curAngle) * radius);
			vertex.y = (float)(centerY + Math.sin(curAngle) * radius);
			vertices[curPart] = vertex;
		}
		return vertices;
	}
