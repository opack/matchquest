...
public class PuzzleLogic {
	...
	/**
	 * Vérifie s'il reste au moins un switch possible
	 * @return
	 */
	...
	private boolean isSwitchPossible(int x, int y) {
		...
		// Si c'est un hyper, alors on peut forcément le switcher
		...
		if (
		(
			// Combinaisons avec 2 alignés et...
			isSameAttribute(curAttribute, x, y + 1)
			// ... un écarté (combinaison en "i")
			&& (isSameAttribute(curAttribute, x, y + 3)
			// ou un sur le côté (combinaison en "L")
				|| isSameAttribute(curAttribute, x - 1, y + 2)
				|| isSameAttribute(curAttribute, x + 1, y + 2))
		)
		||
		(
			// Combinaisons en "parenthèses"
	...
	private boolean match(int x, int y) {
		...
		// Le match réussit s'il y a au moins 3 éléments alignés
	...
	private boolean resolveAlignments(int x, int y) {
		...
		// On marque les attributs comme ayant été matchés pour qu'on ne les match pas à nouveau
